...
[raw, label]=xlsread('test6tdd.csv');
Q = raw(2:2:end,:);                         
I = raw(1:2:end,:);
...
%GD Code
j = 0;
combinedSize = size(combined,1);
%GD Code
while (frame_index < combinedSize)
...
%     disp(ydata); %used to be disp(ydata');
    ...
    for i = 1:chirps_per_frame
        ...
        j = j + 1;
        Distance_Graph(j) = phase_point;
        
        %Double the info and range
        Addition_Graph(j) = phase(range_select) + phase(range_select + 1) + phase(range_select - 1) + .25*phase(range_select + 2) + .25*phase(range_select - 2)  ;

    end
    %GD Code
    
    %GD Code
    %GD plot(time_buffer, plot_buffer); 
    %GD xlim([(curr_time - plot_buffer_size*(Ta/chirps_per_frame))  curr_time]);
...
AveragedFourierTransform = abs(fft(Distance_Graph(1:2000))) + abs(fft(Distance_Graph(2001:4000))) ;
%AveragedFourierTransform = AveragedFourierTransform;
subplot(4,1,1)
plot(Distance_Graph)
...
subplot(4,1,3)
plot(abs(fft(Distance_Graph)));
title('Fourier Transform')
subplot(4,1,4)
plot(AveragedFourierTransform);
